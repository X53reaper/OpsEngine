...
# Build from correct location (parent dir with Dockerfile)
print("=== BUILDING FROM CORRECT LOCATION ===")
result = run("cd /opt/safarizetu-ops-engine && docker build -t safarizetu-ops-engine:latest -f Dockerfile . 2>&1 | tail -15", timeout=300)
print(f"  {result}")
...
result = run("cd /opt/safarizetu-ops-engine/infrastructure && docker compose up -d --force-recreate ops_engine 2>&1", timeout=120)
print(f"  {result}")
