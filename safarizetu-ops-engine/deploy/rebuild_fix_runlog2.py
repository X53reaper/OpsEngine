...
# Build
print("\n=== BUILDING IMAGE ===")
result = run("cd /opt/safarizetu-ops-engine/infrastructure && docker build -t safarizetu-ops-engine:latest -f Dockerfile.ops .. 2>&1 | tail -15", timeout=300)
print(f"  {result}")
...
result = run(f"""docker exec ops_postgres psql -U ops_admin -d safarizetu_ops -c "
